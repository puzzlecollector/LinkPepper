    ...
    if page == "rewards":
        ...
        if lang == "ja":
            return {
                ...
                "og_description": "リンク共有・訪問タスクをこなしてUSDTを獲得。",
...
def _meta_for_leaderboard(lang, request):
    base = _base_url(request)
    common = {"canonical": f"{base}/leaderboard/", "url": f"{base}/leaderboard/"}
    if lang == "ko":
        return {
            "lang": "ko",
            "title": "LinkHash | 리더보드",
...
# ================== PAGES ==================
# NOTE: These *en* endpoints now render using detected language (request.lang),
# so / and /rewards/ adapt for KR/JP users *even without* a prefix.
def home_en(request):
    lang = _lang_from_request(request)
    ctx = {"meta": _meta_for(lang, request), "wallet_user": get_wallet_user(request)}
    return render(request, "home.html", ctx)

def home_ko(request):
    ctx = {"meta": _meta_for("ko", request), "wallet_user": get_wallet_user(request)}
    return render(request, "home.html", ctx)

def home_ja(request):
    ctx = {"meta": _meta_for("ja", request), "wallet_user": get_wallet_user(request)}
    return render(request, "home.html", ctx)

def rewards_en(request):
    lang = _lang_from_request(request)
    ctx = {"meta": _meta_for(lang, request, page="rewards"), "lang": lang, "wallet_user": get_wallet_user(request)}
    return render(request, "rewards.html", ctx)

def rewards_ko(request):
    ctx = {"meta": _meta_for("ko", request, page="rewards"), "lang": "ko", "wallet_user": get_wallet_user(request)}
    return render(request, "rewards.html", ctx)

def rewards_ja(request):
    ctx = {"meta": _meta_for("ja", request, page="rewards"), "lang": "ja", "wallet_user": get_wallet_user(request)}
    return render(request, "rewards.html", ctx)

def rewards_apply_en(request):
    base = _base_url(request)
    # Render in detected language so /rewards/apply/ also adapts
    lang = _lang_from_request(request)
    # Build per-lang meta quickly:
    if lang == "ko":
        meta = {
            "lang": "ko",
...
def events_en(request):
    lang = _lang_from_request(request)
    return render(request, "events.html", {"meta": _meta_for_events(lang, request), "wallet_user": get_wallet_user(request)})

def events_ko(request):
    return render(request, "events.html", {"meta": _meta_for_events("ko", request), "wallet_user": get_wallet_user(request)})

def events_ja(request):
    return render(request, "events.html", {"meta": _meta_for_events("ja", request), "wallet_user": get_wallet_user(request)})
...
def submit_link(request, slug):
    ...
    try:
        Submission.objects.create(
            campaign=campaign, user=user, comment=comment, wallet_used=wallet,
            post_url=post_url, status=Submission.Status.PENDING, proof_score=0,
        )
...
def submit_visit(request, slug):
    ...
    try:
        Submission.objects.create(
            campaign=campaign, user=user, wallet_used=wallet,
            visited_url=visited_url, code_entered=code, status=Submission.Status.PENDING,
        )
    except IntegrityError:
        pass
...
def _leaderboard_from_db(since_dt):
    ...
    for a in agg:
        ...
        rows.append({
            "user_id": a["user_id"], "address": a["user__address"] or "",
